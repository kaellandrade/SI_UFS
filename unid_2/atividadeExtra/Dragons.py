...
class MinHeap:
    def __init__(self, key=lambda x: x, arr=[]):
        """
        Nesta implementação o Heap inicializa com um valor;
        """
    ...
    def heap_increse_key(self, i):
        """
        Move o valor para cima para manter a propriedade do heap.
        """
    ...
    def min_heapify(self, i):
        ...
        if(l <= self.current_size and self.key(self.heap_list[l]) < self.key(self.heap_list[i])):
            ...
            menor = i
        if(r <= self.current_size and self.key(self.heap_list[r]) < self.key(self.heap_list[menor])):
            menor = r
        if menor != i:
...
def game() -> str:
    ...
    # Coletando os dados da primeira linha.
    forca_kirito, n_drag = list(map(int, input().split()))

    heap_todos_drags = MinHeap()  # Lista tuplas armazena todos dragões (forca_d, bonus_d)

    #Vamos popular nosso Heap com todos os dragões
    for _ in range(n_drag):
        forca_d, bonus_d = list(map(int, input().split()))
        heap_todos_drags.insert((forca_d, bonus_d))
    
    while(heap_todos_drags.current_size): #Começa o jogo
        fd, bd = heap_todos_drags.heap_extract_min()
        if(fd >= forca_kirito):
            return 'NO'
        forca_kirito += bd
    return 'YES'
