...
'''
Graph recebe uma lista de vértices, label (nome) e uma booleano
para verificar se é direcionado (False por padrão);
'''
class Graph(Generic[V]):
    def __init__(self, vertices: List[V] = [], label: str = '' , directed: bool = False) -> None:
        self.label = label;
    ...
    def edge_count(self) -> int:
        if(self.directed):
            ...
            return sum(map(len, self._edges))/2 
    ...
    '''
    Método interno para adicionar aresta.
    Trabalha com grafos direcionados e não direcionados.
    '''
